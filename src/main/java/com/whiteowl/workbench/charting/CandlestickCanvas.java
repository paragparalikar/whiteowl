...
public final class CandlestickCanvas extends Canvas {
    ...
    private void handleDrawingClick() {
        long timestamp = getHoveredTimestamp();
    ...
    public long getHoveredTimestamp() {
        ...
        return lastTs + overshoot * activeTimeframe.getSeconds();
    }
    ...
    private CoordinateMapper buildMapper(float[] priceRange) {
        ...
        return new CoordinateMapper() {
            ...
            public double toX(long timestamp) {
                int barIndex = findBarIndexByTimestamp(bars, cachedFrom, timestamp, tfSeconds);
            ...
            public int countBars(long ts1, long ts2) {
                int idx1 = findBarIndexByTimestamp(bars, cachedFrom, ts1, tfSeconds);
                int idx2 = findBarIndexByTimestamp(bars, cachedFrom, ts2, tfSeconds);
                return Math.abs(idx2 - idx1);
            }
        };
    }

    private static int findBarIndexByTimestamp(Bars bars, int cachedFrom, long timestamp, long tfSeconds) {
        if (bars == null || bars.size() == 0) return 0;
        long lastTs = bars.getTimestamp(bars.size() - 1);
        if (timestamp > lastTs && tfSeconds > 0) {
            int overshoot = (int) ((timestamp - lastTs) / tfSeconds);
