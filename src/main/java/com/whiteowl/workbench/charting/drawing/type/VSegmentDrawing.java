...
public final class VSegmentDrawing implements DrawingType {

    private static final String LABEL = "V Segment";
    ...
    public void render(GraphicsContext gc, Drawing d, CoordinateMapper mapper,
        ...
        gc.strokeLine(x1, top, x1, bottom);
        drawAnchorDot(gc, x1, y1);
        drawAnchorDot(gc, x1, y2);
    }
    ...
    public int hitTest(Drawing d, double px, double py, CoordinateMapper mapper,
        ...
        if (hitTestPoint(px, py, ax1, ay2)) return 2;
        if (hitTestLineSegment(px, py, ax1, ay1, ax1, ay2)) return 0;
        return -1;
    }
