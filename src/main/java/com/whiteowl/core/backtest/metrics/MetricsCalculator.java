package com.whiteowl.core.backtest.metrics;

import com.whiteowl.core.backtest.model.BacktestConfig;
import com.whiteowl.core.backtest.model.BacktestReport;
...
public final class MetricsCalculator {
    ...
    public static BacktestReport compute(List<BacktestResult> results, BacktestConfig config) {
    ...
    private static List<TradeRecord> mergeAndSortTrades(List<BacktestResult> results) {
    ...
    private static EquityCurve mergeEquityCurves(List<BacktestResult> results, float initialCapital) {
        if (results.isEmpty()) {
            return new EquityCurve(new float[]{initialCapital}, new long[]{0L}, 1);
        }
        if (results.size() == 1) {
            return results.get(0).getEquityCurve();
        }
        EquityCurve longest = results.get(0).getEquityCurve();
