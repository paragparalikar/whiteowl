...
import com.whiteowl.workbench.collection.ActiveTradesResolver;
import com.whiteowl.workbench.collection.CollectionResolver;
import com.whiteowl.core.portfolio.service.PortfolioQuantityProvider;
import com.whiteowl.workbench.review.model.ReviewConfig;
...
import com.whiteowl.scripting.indicator.script.IndicatorScriptConstants;
import com.whiteowl.scripting.ranker.script.RankerScriptConstants;
import com.whiteowl.scripting.screener.script.ScreenerScriptConstants;
import com.whiteowl.scripting.script.ScriptDescriptor;
import com.whiteowl.scripting.script.ScriptRepository;
import com.whiteowl.scripting.script.ScriptType;
import com.whiteowl.workbench.backtest.BacktestPane;
import com.whiteowl.workbench.orbbacktest.RotationalOrbBacktestPane;
import com.whiteowl.workbench.orbbacktest.RotationalOrbResultPane;
import com.whiteowl.workbench.orboptimizer.OrbOptimizerPane;
import com.whiteowl.workbench.orboptimizer.OptimizationResultPane;
...
public final class WorkbenchView {
    ...
    public WorkbenchView(Stage stage, Context context) {
        ...
        this.chartPane = new ChartPane(context.getBarsRepository(), context.getScripRepository(),
                ...
                context.getBreadthComputer(), context.getGroupRepository(),
                context.getBreadthFormulaRegistry(),
                context.getRsComputer(), context.getRsFormulaRegistry());
    ...
    private void buildLayout(Stage stage) {
        ...
        this.statusBar = new StatusBar(context.getActiveAccountManager(), context.getAccountService());
    ...
    private GroupPane ensureGroupPane() {
        if (groupPane == null) {
            ...
            groupPane.setBreadthComputer(context.getBreadthComputer());
            groupPane.setRsComputer(context.getRsComputer());
    ...
    private BacktestPane ensureBacktestPane() {
        if (backtestPane == null) {
            backtestPane = new BacktestPane(context.getScripRepository(), context.getBarsRepository(),
                    strategyScriptRepo, context.getBreadthComputer(), context.getBreadthFormulaRegistry(),
                    context.getRsComputer(), context.getRsFormulaRegistry());
    ...
    private OrbOptimizerPane ensureOrbOptimizerPane() {
        if (orbOptimizerPane == null) {
            orbOptimizerPane = new OrbOptimizerPane(context.getBarsRepository(),
                    context.getGroupRepository());
            orbOptimizerPane.setOnOptimizationComplete(this::openOptimizationResultTab);
        }
        return orbOptimizerPane;
    }

    private RotationalOrbBacktestPane ensureOrbBacktestPane() {
        if (orbBacktestPane == null) {
            orbBacktestPane = new RotationalOrbBacktestPane(context.getBarsRepository(),
                    context.getGroupRepository());
            orbBacktestPane.setOnBacktestComplete(this::openOrbBacktestResultTab);
        }
    ...
    private void openOrbBacktestResultTab(
        ...
        resultPane.displayReport(report);
        Tab tab = new Tab("ORB Backtest", resultPane);
    ...
    private void openOptimizationResultTab(OrbOptimizerPane.OptimizationRunResult runResult) {
        OptimizationResultPane resultPane = new OptimizationResultPane(runResult);
        String tabTitle = "Opt: " + runResult.params().stream()
                .map(p -> p.name())
                .reduce((a, b) -> a + " x " + b)
                .orElse("Result");
        Tab tab = new Tab(tabTitle, resultPane);
        tab.setGraphic(createTabIcon(FluentUiRegularAL.DATA_AREA_24));
        rightTabPane.getTabs().add(tab);
        rightTabPane.getSelectionModel().select(tab);
    }
    ...
    private MenuBar buildMenuBar(Stage stage) {
        ...
        MenuItem viewOrbBacktest = createViewMenuItem(TAB_ORB_BACKTEST, FluentUiRegularAL.DATA_BAR_VERTICAL_20,
    ...
    private void openNoteEditor(com.whiteowl.workbench.note.model.Note note) {
        for (Tab tab : rightTabPane.getTabs()) {
    ...
    private void clearChartOrderService() {
        ...
        chartPane.setLiveDataManager(null);
        chartPane.setBrokerAdapter(null);
        if (orderBookPanel != null) orderBookPanel.setChartOrderService(null);
        if (statusBar != null) statusBar.setLiveStreaming(false);
    }

    private void wireChartOrderService(BrokerAdapter brokerAdapter) {
        ...
        if (statusBar != null) statusBar.setLiveStreaming(true);
        subscribePortfolioInstruments(brokerAdapter);
    }

    private void subscribePortfolioInstruments(BrokerAdapter brokerAdapter) {
        java.util.concurrent.CompletableFuture.runAsync(() -> {
            try {
                java.util.Set<Scrip> scrips = new java.util.LinkedHashSet<>();
                brokerAdapter.fetchPositions().stream()
                        .map(com.whiteowl.core.portfolio.model.Position::getScrip)
                        .filter(java.util.Objects::nonNull)
                        .forEach(scrips::add);
                brokerAdapter.fetchHoldings().stream()
                        .map(com.whiteowl.core.portfolio.model.Holding::getScrip)
                        .filter(java.util.Objects::nonNull)
                        .forEach(scrips::add);
                if (!scrips.isEmpty()) {
                    brokerAdapter.subscribe(scrips);
                }
            } catch (Exception e) {
                // Portfolio instrument subscription is best-effort
            }
        });
    }
