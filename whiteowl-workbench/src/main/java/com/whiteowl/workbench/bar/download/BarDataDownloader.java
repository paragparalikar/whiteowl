...
public final class BarDataDownloader {
    ...
    private void executeForwardDownload(DownloadQuery query, int attempt) {
        ...
        try {
            List<Candle> candles = brokerAdapter.fetchHistoricalData(
                    scrip, timeframe, query.getFrom(), query.getTo());
            List<Candle> verified = verifier.verify(scrip.getId(), timeframe, candles);
    ...
    private void appendCandles(String scripId, Timeframe timeframe, List<Candle> candles) {
        if (candles.isEmpty()) return;
    ...
    private void prependCandles(String scripId, Timeframe timeframe, List<Candle> candles) {
        if (candles.isEmpty()) return;
        Bars bars = toCandleBars(scripId, timeframe, candles);
        try {
    ...
    private Bars toCandleBars(String scripId, Timeframe timeframe, List<Candle> candles) {
