...
public final class BarDataProvider {
    ...
    /**
     * Appends a completed live bar to the cached data and increments totalBars.
     * Used when a new bar is finalized from live tick data.
     */
    public void appendLiveBar(long ts, float o, float h, float l, float c, long vol) {
        if (cachedBars != null) {
            cachedBars.append(ts, o, h, l, c, vol);
            cachedTo++;
        }
        totalBars++;
    }
    ...
    /**
     * Updates the last bar in the cache with live in-progress data.
     * Used when the current bar's OHLC changes from new ticks.
     */
    public void updateLastBar(long ts, float o, float h, float l, float c, long vol) {
        if (cachedBars != null && cachedBars.size() > 0) {
            cachedBars.setBar(cachedBars.size() - 1, ts, o, h, l, c, vol);
        }
    }
