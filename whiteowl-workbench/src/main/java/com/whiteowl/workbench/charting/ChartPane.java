...
public final class ChartPane extends BorderPane {
    ...
    public ChartPane(BarsRepository barsRepository, ScripRepository scripRepository,
                     ...
                     BreadthFormulaRegistry breadthFormulaRegistry,
        ...
        this.indicatorRegistry = new IndicatorRegistry(barsRepository, indicatorScriptRepo,
                breadthComputer, groupRepository, breadthFormulaRegistry,
                rsComputer, rsFormulaRegistry);
    ...
    private com.whiteowl.core.tick.model.TickListener activeLtpListener;
    private volatile float lastLtp;
    private long lastLiveRenderTime;
    private static final long MIN_RENDER_INTERVAL_MS = 250;

    public void setLiveDataManager(com.whiteowl.core.bar.aggregation.LiveDataManager manager) {
        this.liveDataManager = manager;
    }

    public void setBrokerAdapter(com.whiteowl.core.broker.BrokerAdapter adapter) {
    ...
    private void subscribeForLiveData(Scrip scrip) {
        ...
        if (liveDataManager != null) {
            activeBarListener = new com.whiteowl.core.bar.aggregation.BarCompletionListener() {
                @Override
                public void onBarCompleted(String scripId, com.whiteowl.core.bar.model.Timeframe timeframe,
                                           long timestamp, float open, float high, float low, float close, long volume) {
                    if (!scripId.equals(dataProvider.getScripId()) || timeframe != dataProvider.getTimeframe()) return;
                    Platform.runLater(() -> {
                        dataProvider.appendLiveBar(timestamp, open, high, low, close, volume);
                        viewport.configure(dataProvider.getTotalBars());
                        try {
                            lastBars = dataProvider.fetchBars(viewport.getStartIndex(), viewport.getEndIndex());
                        } catch (Exception e) {
                            log.warn("Failed to fetch bars after live bar completion", e);
                        }
                        canvas.render();
                    });
                }
                ...
                @Override
                public void onBarUpdated(String scripId, com.whiteowl.core.bar.model.Timeframe timeframe,
                                         long timestamp, float open, float high, float low, float close, long volume) {
                    if (!scripId.equals(dataProvider.getScripId()) || timeframe != dataProvider.getTimeframe()) return;
                    long now = System.currentTimeMillis();
                    if (now - lastLiveRenderTime < MIN_RENDER_INTERVAL_MS) return;
                    lastLiveRenderTime = now;
                    Platform.runLater(() -> {
                        dataProvider.updateLastBar(timestamp, open, high, low, close, volume);
                        canvas.render();
                    });
                }
            };
            liveDataManager.addBarListener(activeBarListener);

            activeLtpListener = tick -> {
                if (activeScrip != null && activeScrip.getId().equals(tick.getScripId())) {
                    lastLtp = tick.getLastTradedPrice();
                    long now = System.currentTimeMillis();
                    if (now - lastLiveRenderTime < MIN_RENDER_INTERVAL_MS) return;
                    lastLiveRenderTime = now;
                    Platform.runLater(() -> {
                        canvas.setLivePrice(lastLtp);
                        canvas.render();
                    });
                }
            };
            liveDataManager.addTickListener(activeLtpListener);
        }
    }

    private void unsubscribeLiveData() {
        if (liveDataManager != null) {
            if (activeBarListener != null) {
                liveDataManager.removeBarListener(activeBarListener);
                activeBarListener = null;
            }
            if (activeLtpListener != null) {
                liveDataManager.removeTickListener(activeLtpListener);
                activeLtpListener = null;
            }
        }
        lastLtp = 0;
    }
    ...
    private HBox buildDrawingToolButtons() {
        HBox box = buildButtonGroup();
        for (DrawingTool tool : new DrawingTool[]{DrawingTool.H_SEGMENT, DrawingTool.SLANT_SEGMENT, DrawingTool.TEXT}) {
            ToggleButton btn = new ToggleButton();
    ...
    private void openSubChartSettingsDialog(ActiveSubChartIndicator ai) {
        ...
        new IndicatorSettingsDialog(ai, scripRepository, groupRepository, () -> {
            computeSubChartIndicators();
            refreshIndicatorList();
            renderSubCharts();
        }).show(owner);
