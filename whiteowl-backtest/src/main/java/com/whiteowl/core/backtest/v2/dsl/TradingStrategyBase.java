...
import com.whiteowl.core.backtest.v2.indicator.RvolIndicator;
...
import com.whiteowl.core.backtest.v2.indicator.SupertrendIndicator;
import com.whiteowl.core.backtest.v2.indicator.TwapIndicator;
...
public abstract class TradingStrategyBase extends Script {
    ...
    // Breadth data — bound by engine before execution
    private final Map<String, FloatSmartValue> breadthValues = new LinkedHashMap<>();

    // RS data — bound by engine before execution
    private final Map<String, FloatSmartValue> rsValues = new LinkedHashMap<>();
    ...
    /**
     * Bind pre-computed breadth data. Called by BacktestEngine before strategy execution.
     */
    public void bindBreadthData(String name, FloatSmartValue value) {
        breadthValues.put(name, value);
    }

    /**
     * Access a pre-computed breadth series by name.
     * Usage in Groovy: def b = breadth("NIFTY50_AdvanceDecline")
     * Then: b.value(), b[0], b[-1], sma(b, 20), crossover(b, someOther)
     */
    public FloatSmartValue breadth(String name) {
        FloatSmartValue sv = breadthValues.get(name);
        if (sv == null) {
            throw new IllegalArgumentException("Breadth data not found: " + name
                    + ". Available: " + breadthValues.keySet());
        }
        return sv;
    }

    /**
     * Bind pre-computed RS data. Called by BacktestEngine before strategy execution.
     */
    public void bindRSData(String name, FloatSmartValue value) {
        rsValues.put(name, value);
    }

    /**
     * Access a pre-computed RS series by name.
     * Usage in Groovy: def strength = rs("NIFTY50")
     * Then: strength.value(), strength[0], strength[-1], sma(strength, 20)
     */
    public FloatSmartValue rs(String name) {
        FloatSmartValue sv = rsValues.get(name);
        if (sv == null) {
            throw new IllegalArgumentException("RS data not found: " + name
                    + ". Available: " + rsValues.keySet());
        }
        return sv;
    }
    ...
    public FloatSmartValue[] supertrend(int period, float multiplier) {
        String sig = "supertrend:" + period + ":" + multiplier;
        if (!setupPhase) {
            IndicatorNode node = indicatorGraph.findBySignature(sig);
            if (node == null) {
                throw new IllegalStateException("Indicator not found: " + sig);
            }
            SupertrendIndicator st = (SupertrendIndicator) node.getDerivedIndicator();
            return new FloatSmartValue[]{ st.getSupertrendOutput(), st.getDirectionOutput() };
        }
        FloatSmartValue stOutput = new FloatSmartValue(maxBars);
        FloatSmartValue dirOutput = new FloatSmartValue(maxBars);
        SupertrendIndicator indicator = new SupertrendIndicator(high, low, close, period, multiplier, stOutput, dirOutput);
        IndicatorNode node = IndicatorNode.ofDerived(stOutput, indicator, sig);
        indicatorGraph.addNode(node);
        return new FloatSmartValue[]{ stOutput, dirOutput };
    }

    public FloatSmartValue twap() {
        String sig = "twap";
        if (!setupPhase) {
            IndicatorNode node = indicatorGraph.findBySignature(sig);
            if (node == null) {
        ...
        FloatSmartValue output = new FloatSmartValue(maxBars);
        TwapIndicator indicator = new TwapIndicator(high, low, close, timestamp, output);
        IndicatorNode node = IndicatorNode.ofDerived(output, indicator, sig);
        indicatorGraph.addNode(node);
        return output;
    }
    ...
    public FloatSmartValue rvol(int period) {
        String sig = "rvol:" + period;
        if (!setupPhase) {
            IndicatorNode node = indicatorGraph.findBySignature(sig);
            if (node == null) {
                throw new IllegalStateException("Indicator not found: " + sig);
        ...
        FloatSmartValue output = new FloatSmartValue(maxBars);
        RvolIndicator indicator = new RvolIndicator(volume, output, period);
        IndicatorNode node = IndicatorNode.ofDerived(output, indicator, sig);
        indicatorGraph.addNode(node);
        return output;
    }
