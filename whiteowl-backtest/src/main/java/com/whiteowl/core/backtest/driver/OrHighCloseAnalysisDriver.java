...
/**
 * Analyzes the distance between OR high and OR close for all trades
 * produced by the default strategy configuration.
 *
 * <p>This analysis addresses fill probability: when placing limit buy orders
 * at OR high after seeing the OR close, how far does price need to move
 * to fill the order?</p>
 *
 * <h3>Usage</h3>
 * <pre>
 *   mvn compile exec:java -pl whiteowl-backtest \
 *       -Dexec.mainClass=com.whiteowl.core.backtest.rotational.feature.OrHighCloseAnalysisDriver
 * </pre>
 */
public final class OrHighCloseAnalysisDriver {

    private static final Logger log = LoggerFactory.getLogger(OrHighCloseAnalysisDriver.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        // ── Feature ───────────────────────────────────────────────────
        List<TradeFeature> features = List.of(new OrHighToCloseDistanceFeature());

        // ── Analyzer ──────────────────────────────────────────────────
        List<FeatureAnalyzer> analyzers = List.of(
                new BucketAnalyzer("OR_HIGH_CLOSE_PCT", 0.10,
        ...
        log.info("==============================================================");
        log.info("  OR High-to-Close Distance Analysis");
        log.info("==============================================================");
        log.info("{}", config);
        log.info("");
        ...
        Path csvPath = Path.of(System.getProperty("user.home"), ".whiteowl",
                "analysis", "or_high_close_distance.csv");
        ...
        // Long-side analysis (most relevant for limit buy at OR high)
        log.info("==============================================================");
        log.info("  LONG TRADES ONLY (limit buy at OR high)");
        log.info("==============================================================");
        ...
        // Short-side analysis (limit sell at OR low — inverse relevance)
        log.info("==============================================================");
    ...
    /**
     * Print a custom analysis focused on fill probability.
     * Since all trades in the backtest were triggered (price did cross OR high/low),
     * this feature value tells us how far OR close was from the breakout level.
     */
    private static void printFillProbabilityAnalysis(
        ...
        log.info("");
        log.info("==============================================================");
        log.info("  Fill Probability Analysis");
        log.info("  (How far is OR close from OR high, as % of OR close)");
        log.info("==============================================================");
        log.info("");
        ...
        for (int i = 0; i < trades.size(); i++) {
            Double val = featureValues.get(i).get("OR_HIGH_CLOSE_PCT");
        ...
        log.info("For LONG entries:");
        log.info("  - We place limit buy at OR high after seeing OR close");
        log.info("  - OR_HIGH_CLOSE_PCT = how far below OR high the OR closed");
        log.info("  - Small values = OR closed near high = limit likely fills quickly");
        log.info("  - Large values = OR closed well below high = needs bigger rally to fill");
        log.info("");
        log.info("For SHORT entries:");
        log.info("  - Analogous: distance between OR high and OR close");
        log.info("  - For shorts, the relevant metric would be OR close to OR low");
        log.info("  - But this feature shows the same OR candle structure");
        log.info("");
    }
