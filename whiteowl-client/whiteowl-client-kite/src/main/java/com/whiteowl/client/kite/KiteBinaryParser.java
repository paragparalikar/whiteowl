...
final class KiteBinaryParser {
    ...
    private KiteTick parseIndex(byte[] bin, int token) {
        ...
        tick.setHigh(readFloat(bin, 8) / DIVISOR_DEFAULT);
        tick.setLow(readFloat(bin, 12) / DIVISOR_DEFAULT);
        tick.setOpen(readFloat(bin, 16) / DIVISOR_DEFAULT);
        tick.setClose(readFloat(bin, 20) / DIVISOR_DEFAULT);
        return tick;
    }
    ...
    private KiteTick parseQuote(byte[] bin, int token, int divisor) {
        ...
        tick.setOpen(readFloat(bin, 28) / divisor);
        tick.setHigh(readFloat(bin, 32) / divisor);
        tick.setLow(readFloat(bin, 36) / divisor);
        tick.setClose(readFloat(bin, 40) / divisor);
        tick.setLastTradedTime(System.currentTimeMillis());
        return tick;
    }
    ...
    private List<byte[]> splitPackets(byte[] bin) {
        if (bin == null || bin.length < 2) return List.of();
        ...
        for (int i = 0; i < packetCount; i++) {
            ...
            if (offset + 2 + size > bin.length) break;
